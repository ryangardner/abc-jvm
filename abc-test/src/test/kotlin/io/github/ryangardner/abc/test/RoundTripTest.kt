...
public class RoundTripTest {
    ...
    public fun `test round trip fidelity`(file: File) {
        ...
        val roundTrippedTunes: List<AbcTune> =
            try {
        ...
        originalTunes.forEachIndexed { tuneIndex: Int, originalTune: AbcTune ->
            ...
            // Basic header checks
            assertTrue(
                originalTune.header.reference == roundTrippedTune.header.reference,
                FidelityReporter.reportRoundTripFailure(
                    file,
                    tuneIndex,
                    originalTune,
                    roundTrippedTune,
                    "Reference mismatch: expected ${originalTune.header.reference}, got ${roundTrippedTune.header.reference}",
                    originalAbc,
                    serializedBook,
                ),
            )
            assertTrue(
                originalTune.header.title == roundTrippedTune.header.title,
                FidelityReporter.reportRoundTripFailure(
                    file,
                    tuneIndex,
                    originalTune,
            ...
            assertTrue(
                originalTune.header.key == roundTrippedTune.header.key,
                FidelityReporter.reportRoundTripFailure(
                    file,
                    tuneIndex,
                    originalTune,
            ...
            assertTrue(
                originalBodyElements == roundTrippedBodyElements,
                FidelityReporter.reportRoundTripFailure(
                    file,
                    tuneIndex,
                    originalNormalizedTune,
                    roundTrippedNormalizedTune,
                    "Body structural mismatch (Normalized)",
                    originalAbc,
                    serializedBook,
                ),
            )
            ...
            assertTrue(
                originalPreamble == roundTrippedPreamble,
                FidelityReporter.reportRoundTripFailure(
                    file,
                    tuneIndex,
                    originalNormalizedPreambleTune,
                    roundTrippedNormalizedPreambleTune,
                    "Preamble structural mismatch (Normalized)",
                    originalAbc,
                    serializedBook,
                ),
            )
            ...
            assertTrue(
                originalInterpreted.voices.size == roundTrippedInterpreted.voices.size,
                FidelityReporter.reportRoundTripFailure(
                    file,
                    tuneIndex,
                    originalTune,
                    roundTrippedTune,
                    "Interpreted voice count mismatch",
                    originalAbc,
                    serializedBook,
                ),
            )
            ...
            originalInterpreted.voices.forEach { (voiceId, originalNotes) ->
                ...
                assertTrue(
                    originalNotes.size == roundTrippedNotes.size,
                    FidelityReporter.reportRoundTripFailure(
                        file,
                        tuneIndex,
                        originalTune,
                        roundTrippedTune,
                        "Voice $voiceId element count mismatch",
                        originalAbc,
                        serializedBook,
                    ),
                )
                ...
                originalNotes.forEachIndexed { noteIndex, originalNote ->
                    ...
                    assertTrue(
                        originalNote.pitches.map { it.midiNoteNumber }.sorted() ==
                            roundTrippedNote.pitches.map { it.midiNoteNumber }.sorted(),
                        FidelityReporter.reportRoundTripFailure(
                            file,
                            tuneIndex,
                            originalTune,
                            roundTrippedTune,
                            "Voice $voiceId Note $noteIndex pitch mismatch",
                            originalAbc,
                            serializedBook,
                        ),
                    )
                    assertEquals(
                        ...
                        FidelityReporter.reportRoundTripFailure(
                            ...
                            originalTune,
                            roundTrippedTune,
                            "Voice $voiceId Note $noteIndex duration mismatch",
                            originalAbc,
                            serializedBook,
                        ),
                    )
