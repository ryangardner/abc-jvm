...
public class RegressionHeavyTest {
    ...
    public fun `test regression tunes fidelity`(file: File) {
        ...
        originalTunes.forEachIndexed { tuneIndex, originalTune ->
            ...
            assertEquals(
                ...
                "[${file.name}] Tune $tuneIndex Voice count mismatch",
            )
            ...
            originalInterpreted.voices.forEach { (voiceId, originalNotes) ->
                ...
                assertEquals(
                    ...
                    "[${file.name}] Tune $tuneIndex Voice $voiceId element count mismatch",
                )
                ...
                originalNotes.forEachIndexed { noteIndex, originalNote ->
                    ...
                    assertEquals(
                        ...
                        "[${file.name}] Tune $tuneIndex Voice $voiceId Note $noteIndex pitch mismatch",
                    )
                    assertEquals(
                        originalNote.duration.toDouble(),
                        roundTrippedNote.duration.toDouble(),
                        0.001,
                        "[${file.name}] Tune $tuneIndex Voice $voiceId Note $noteIndex duration mismatch",
                    )
                }
            }
        }
