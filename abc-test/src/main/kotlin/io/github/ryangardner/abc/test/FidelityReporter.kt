...
public object FidelityReporter {
    public fun reportMeasureErrors(
        file: File,
        tuneIndex: Int,
        errors: List<MeasureError>,
        originalAbc: String,
    ): String {
        ...
        errors.take(10).forEach { error ->
            sb.append(
                "  [Voice: ${error.voice}] Measure ${error.measureIndex}: Expected ${error.expectedDuration}, but got ${error.actualDuration}\n",
            )
        }
        if (errors.size > 10) {
    ...
    public fun reportRoundTripFailure(
        ...
        if (firstDiffIdx != -1) {
            ...
            if (rtElements.size > origElements.size) {
                ...
                rtElements
                    .subList(size, rtElements.size)
                    .take(5)
                    .map { it.withoutLocation() }
                    .forEach { sb.append("  + $it\n") }
            } else {
                sb.append("Missing elements in ACTUAL (present in EXPECTED):\n")
                origElements
                    .subList(size, origElements.size)
                    .take(5)
                    .map { it.withoutLocation() }
                    .forEach { sb.append("  - $it\n") }
            }
    ...
    public fun reportUnrecognizedCharacters(
        file: File,
        tuneIndex: Int,
        tune: AbcTune,
    ): String {
        ...
        val unrecognized =
            tune.body.elements.filter {
                it is io.github.ryangardner.abc.core.model.SpacerElement &&
                    (
                        it.text.length == 1 && !it.text[0].isWhitespace() && it.text[0] != 'y' && it.text[0] != '$' && it.text[0] != '>' &&
                            it.text[0] != '<' &&
                            it.text[0] != '`' &&
                            it.text[0] != ':' &&
                            it.text[0] != ']' &&
