...
public class Timeline(
     ...
     */
    public fun getEventsAt(beat: Double): List<TimeEvent> = events.filter { it.beat == beat }
    ...
    public fun getChordsAt(beat: Double): List<TimeEvent> =
        events.filter { it.beat == beat && (it.note.annotations.isNotEmpty() || it.note.pitches.size > 1) }
     ...
     */
    public fun getEventsInRange(
        startBeat: Double,
        endBeat: Double,
    ): List<TimeEvent> = events.filter { it.beat in startBeat..endBeat }
    ...
    public fun totalBeats(): Double =
        events
            .maxByOrNull { it.beat + it.note.semanticDuration.toDouble() }
