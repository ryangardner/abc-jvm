...
public data class KeyCandidate(
    ...
    public val tonicName: String get() =
        buildString {
...
public object CircleOfFifths {
    ...
    private fun getModeOffsetInSemitones(mode: KeyMode): Int =
        when (mode) {
    ...
    private fun getModeStepOffset(mode: KeyMode): Int =
        when (mode) {
            KeyMode.MAJOR, KeyMode.IONIAN -> 0
            KeyMode.DORIAN -> 1
            KeyMode.PHRYGIAN -> 2
    ...
    public fun getAccidentalForStep(
        step: NoteStep,
        k: Int,
    ): Int {
    ...
    public fun stepToSemitones(step: NoteStep): Int =
        when (step) {
            NoteStep.C -> 0
            NoteStep.D -> 2
            NoteStep.E -> 4
    ...
    public fun semitonesToAccidental(semitones: Int): Accidental? =
        when (semitones) {
            1 -> Accidental.SHARP
            -1 -> Accidental.FLAT
